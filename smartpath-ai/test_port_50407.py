#!/usr/bin/env python3
"""
Test the simple database server on port 50407
"""
import requests

def test_server():
    print('🧪 Testing simple database server on port 50407...')
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if chat_response.status_code == 200:
                ...
                print(f'📊 Response data keys: {list(chat_data.keys())}')
                ...
                if chat_data.get('success'):
                    ...
                    chat_response2 = requests.post('http://localhost:50407/chat', data={
                    ...
                    if chat_response2.status_code == 200:
                        ...
                        if chat_data2.get('success'):
                            ...
                            print(f'🤖 Second Response: {ai_response2}')
                            print('\n🎉 SIMPLE DATABASE SERVER IS WORKING PERFECTLY!')
                            return True
                        else:
                            print(f'❌ Second Chat Error: {chat_data2.get("ai_response", "Unknown")}')
                    else:
                        print(f'❌ Second Chat HTTP Error: {chat_response2.status_code}')
                    
                else:
                    print(f'❌ Chat Error: {chat_data.get("ai_response", "Unknown")}')
                    print(f'📊 Full response: {chat_data}')
            else:
...
if __name__ == "__main__":
    if test_server():
        ...
        print("\n🚀 Your Streamlit app is now connected to port 50407!")
